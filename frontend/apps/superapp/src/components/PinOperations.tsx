...
const inputStyle: React.CSSProperties = { background: '#0A0A0A', border: '1px solid #2A2A2A', borderRadius: 6, padding: '8px 12px', color: '#FFFFFF', fontSize: 13, width: '100%', boxSizing: 'border-box' };
const btnStyle = (enabled: boolean): React.CSSProperties => ({ background: enabled ? '#3B82F6' : '#2A2A2A', color: enabled ? '#FFFFFF' : '#666666', border: 'none', borderRadius: 6, padding: '8px 16px', cursor: enabled ? 'pointer' : 'not-allowed', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, width: '100%' });
      ...
      <div style={{ marginBottom: 16, paddingBottom: 16, borderBottom: '1px solid #2A2A2A' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <button onClick={handleEncrypt} disabled={!pin || !pan || !!loading} style={btnStyle(!!pin && !!pan && !loading)} data-testid="encrypt-btn">
            {loading === 'encrypt' ? <RefreshCw size={14} style={{ animation: 'spin 1s linear infinite' }} /> : <Lock size={14} />}
            {loading === 'encrypt' ? 'Encrypting...' : 'Encrypt PIN'}
          </button>
          {encryptedPinBlock && <div style={{ marginTop: 4, background: '#0A0A0A', padding: 8, borderRadius: 4, fontFamily: 'monospace', fontSize: 12, color: '#22C55E', wordBreak: 'break-all' }}>{encryptedPinBlock}</div>}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16, paddingBottom: 16, borderBottom: '1px solid #2A2A2A' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <button onClick={handleDecrypt} disabled={!decryptBlock || !decryptPan || !!loading} style={btnStyle(!!decryptBlock && !!decryptPan && !loading)} data-testid="decrypt-btn">
            {loading === 'decrypt' ? <RefreshCw size={14} style={{ animation: 'spin 1s linear infinite' }} /> : 'Decrypt PIN Block'}
          </button>
          {decryptedPin && <div style={{ marginTop: 4, background: '#0A0A0A', padding: 8, borderRadius: 4, fontFamily: 'monospace', fontSize: 12, color: '#22C55E' }}>PIN: {decryptedPin}</div>}
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <button onClick={handleVerify} disabled={!verifyBlock || !verifyPan || !expectedPin || !!loading} style={btnStyle(!!verifyBlock && !!verifyPan && !!expectedPin && !loading)} data-testid="verify-btn">
            {loading === 'verify' ? <RefreshCw size={14} style={{ animation: 'spin 1s linear infinite' }} /> : 'Verify PIN'}
          </button>
          {verificationResult && (
