...
const RISK_COLORS = { HIGH: '#EF4444', MEDIUM: '#F59E0B', LOW: '#22C55E' } as const;
...
const riskTrendData = [
  { time: '00:00', score: 32 }, { time: '02:00', score: 28 }, { time: '04:00', score: 45 },
  { time: '06:00', score: 52 }, { time: '08:00', score: 67 }, { time: '10:00', score: 72 },
  { time: '12:00', score: 58 }, { time: '14:00', score: 81 }, { time: '16:00', score: 74 },
  { time: '18:00', score: 63 }, { time: '20:00', score: 55 }, { time: '22:00', score: 48 },
];
...
function getLevelBadge(level: RiskLevel) {
  ...
  return <span className={`text-xs font-semibold px-2 py-0.5 rounded border ${colors[level]}`}>{level}</span>;
}
  ...
  return (
    <div className="bg-[#0A0A0A] text-white p-6">
      <header className="flex items-center gap-3 mb-6 border-b border-[#2A2A2A] pb-4">
        ...
        <div className="ml-auto flex items-center gap-2"><Activity className="w-4 h-4 text-[#22C55E]" /><span className="text-sm text-[#A1A1AA]">Live</span></div>
      </header>

      {highRiskCount > 0 && (
        <div className="mb-6 bg-[#EF4444]/10 border border-[#EF4444]/30 rounded-lg p-4 flex items-center gap-3">
          <AlertTriangle className="w-5 h-5 text-[#EF4444] shrink-0" />
          <span className="text-[#EF4444] font-medium">{highRiskCount} HIGH risk transaction{highRiskCount > 1 ? 's' : ''} detected.</span>
        </div>
      )}
        ...
        <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              ...
              <Tooltip contentStyle={{ backgroundColor: '#1A1A1A', border: '1px solid #2A2A2A', borderRadius: '8px' }} itemStyle={{ color: '#FFFFFF' }} />
              <Legend />
            </PieChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg p-5">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={riskTrendData}>
              ...
              <Tooltip contentStyle={{ backgroundColor: '#1A1A1A', border: '1px solid #2A2A2A', borderRadius: '8px' }} itemStyle={{ color: '#FFFFFF' }} labelStyle={{ color: '#A1A1AA' }} />
              <Legend />
              <Line type="monotone" dataKey="score" stroke="#3B82F6" strokeWidth={2} dot={{ fill: '#3B82F6', r: 3 }} activeDot={{ r: 5 }} name="Risk Score" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </section>
      ...
      <section>
        <h2 className="text-lg font-semibold mb-4 flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-[#F59E0B]" />Recent Risk Events</h2>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {recentEvents.map((event) => (
                <tr key={event.id} className="border-b border-[#2A2A2A]/50 hover:bg-[#2A2A2A]/30 transition-colors">
                  <td className="p-3 font-mono text-[#3B82F6]">{event.transactionId}</td>
                  <td className="p-3"><span className="font-semibold" style={{ color: getScoreColor(event.score) }}>{event.score}</span></td>
                  <td className="p-3">{getLevelBadge(event.level)}</td>
                  <td className="p-3">
                    {event.flags.length > 0 ? (
                      <div className="flex gap-1 flex-wrap">
                        {event.flags.map((flag) => <span key={flag} className="text-xs bg-[#0A0A0A] text-[#A1A1AA] px-2 py-0.5 rounded border border-[#2A2A2A]">{flag}</span>)}
                      </div>
