...
function getTypeIcon(type: string) {
  const color = typeColor[type] || '#A1A1AA';
  switch (type) {
    ...
    case 'RiskEvaluated':
      return <Shield className="w-4 h-4" style={{ color }} />;
...
function App() {
  ...
  const verifiedCount = auditLogs.length;
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white p-6">
        ...
        <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg p-5">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-bold text-[#22C55E]">{verifiedCount}</span>
              <span className="text-sm text-[#22C55E]">Verified</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-2xl font-bold text-[#EF4444]">{tamperedCount}</span>
              <span className="text-sm text-[#EF4444]">Tampered</span>
            </div>
          </div>
        </div>
        <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg p-5">
          <div className="text-sm text-[#A1A1AA] mb-1">Events Today</div>
          <div className="text-3xl font-bold">{auditLogs.length}</div>
        </div>
      </section>
      ...
      <section className="mb-6">
            ...
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-lg border font-medium transition-colors ${
                activeFilter === f
                  ...
                  : 'border-[#2A2A2A] text-[#A1A1AA] bg-[#1A1A1A] hover:border-[#A1A1AA]/30'
              }`}
            >
              {f}
            </button>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search by hash, type, or payload..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg py-2 pl-10 pr-4 text-sm text-white placeholder-[#A1A1AA] focus:outline-none focus:border-[#3B82F6]/50 transition-colors"
          />
        </div>
      </section>
      ...
      <section className="mb-8">
        ...
        <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg p-6">
          <div className="relative pl-8">
            ...
            {filtered.length === 0 && (
              <div className="text-center text-[#A1A1AA] py-8">No events match the current filter.</div>
            )}
            ...
            {filtered.map((log, index) => {
              ...
              return (
                <div key={log.id} className="relative pb-6 last:pb-0">
                  {/* Timeline dot */}
                  <div
                    className="absolute -left-8 top-1 w-6 h-6 rounded-full flex items-center justify-center border-2"
                  ...
                  <div className="bg-[#0A0A0A] border border-[#2A2A2A] rounded-lg p-4">
                    ...
                    <div className="flex items-center gap-3 text-xs font-mono mb-2 flex-wrap">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[#A1A1AA]">Hash:</span>
                        <span className="text-[#3B82F6]">{log.hash}</span>
                      </div>
                      {log.prevHash && (
                        <div className="flex items-center gap-1.5">
                          <Link className="w-3 h-3 text-[#2A2A2A]" />
                          <span className="text-[#A1A1AA]">Prev:</span>
                          <span className="text-[#A1A1AA]/70">{log.prevHash}</span>
                        </div>
                      )}
                    </div>

                    {/* Payload */}
                    <div className="text-xs text-[#A1A1AA]/80 font-mono">
      ...
      <section>
        ...
        <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg p-6 overflow-x-auto">
            ...
            {filtered.map((log, index) => {
              ...
              return (
                <div key={log.id} className="flex items-center gap-2">
                  <div
                    className="flex flex-col items-center gap-1 px-3 py-2 rounded-lg border min-w-[90px]"
                  ...
                  {index < filtered.length - 1 && (
                    <svg className="w-6 h-4 shrink-0" viewBox="0 0 24 16">
                      <line x1="0" y1="8" x2="16" y2="8" stroke="#2A2A2A" strokeWidth="2" />
                      <polygon points="16,3 24,8 16,13" fill="#2A2A2A" />
                    </svg>
                  )}
                </div>
              );
