...
import { parseIso8583, buildIso8583, authorizeCard } from '../lib/api';
...
export default function Iso8583Tools() {
  ...
  const [authZpkId] = useState('default-zpk');
  const [authTerminal] = useState('TERM001');
  const [authMerchant] = useState('MERCHANT001');
  ...
  const handleAuthorize = async () => {
    ...
    try {
      const result = await authorizeCard({
        ...
        encryptedPinBlock: authPinBlock || '0000000000000000',
        zpkId: authZpkId,
        terminalId: authTerminal,
        merchantId: authMerchant,
      });
    ...
    <div style={cardStyle}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
        ...
        <div style={{ paddingBottom: 16 }}>
          ...
          <textarea
            placeholder="Paste ISO 8583 message..."
            value={isoInput}
          ...
          <button onClick={handleParse} disabled={!isoInput.trim() || !!loading} style={{ ...btnStyle(!!isoInput.trim() && !loading), marginTop: 8, width: '100%' }} data-testid="parse-btn">
            {loading === 'parse' ? <RefreshCw size={14} className="spin" /> : 'Parse'}
          </button>
          {parsedMessage && (
            <div style={{ marginTop: 8, background: '#0A0A0A', padding: 10, borderRadius: 4, fontSize: 12 }}>
              ...
              {Object.entries(parsedMessage.fields).map(([k, v]) => (
                <div key={k} style={{ color: '#A1A1AA', fontFamily: 'monospace' }}>
                  Field {k}: <span style={{ color: '#FFFFFF' }}>{String(v)}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ paddingBottom: 16 }}>
          ...
          <textarea
            placeholder='{"2": "4111...","4": "000000001000"}'
            value={buildFields}
            onChange={(e) => setBuildFields(e.target.value)}
            style={{ ...inputStyle, minHeight: 80, resize: 'vertical' }}
            data-testid="build-fields-input"
          />
          <button onClick={handleBuild} disabled={!!loading} style={{ ...btnStyle(!loading), marginTop: 8, width: '100%' }} data-testid="build-btn">
            {loading === 'build' ? <RefreshCw size={14} className="spin" /> : 'Build'}
          </button>
          {builtMessage && (
            <div style={{ marginTop: 8, background: '#0A0A0A', padding: 10, borderRadius: 4, fontFamily: 'monospace', fontSize: 12, color: '#22C55E', wordBreak: 'break-all' }}>
              {builtMessage}
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <button onClick={handleAuthorize} disabled={!authPan || !!loading} style={btnStyle(!!authPan && !loading)} data-testid="authorize-btn">
              {loading === 'authorize' ? <RefreshCw size={14} className="spin" /> : 'Authorize Card'}
            </button>
            {authResult && (
