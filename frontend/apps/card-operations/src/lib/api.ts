...
export async function getHsmHealth() {
  const res = await fetch(`${API_BASE}/hsm/health`);
  if (!res.ok) throw new Error('HSM health check failed');
  return res.json();
}

export async function listHsmKeys() {
  const res = await fetch(`${API_BASE}/hsm/keys`);
  if (!res.ok) throw new Error('Failed to list keys');
  return res.json();
}

export async function generateKey(keyType: string, keyId: string) {
  const res = await fetch(`${API_BASE}/hsm/keys/generate`, {
    method: 'POST',
...
export async function encryptPin(pin: string, pan: string, zpkId: string) {
  const res = await fetch(`${API_BASE}/hsm/pin/encrypt`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function decryptPin(encryptedPinBlock: string, pan: string, zpkId: string) {
  const res = await fetch(`${API_BASE}/hsm/pin/decrypt`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function verifyPin(encryptedPinBlock: string, pan: string, zpkId: string, expectedPin: string) {
  const res = await fetch(`${API_BASE}/hsm/pin/verify`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function buildIso8583(mti: string, fields: Record<string, string>) {
  const res = await fetch(`${API_BASE}/iso8583/build`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function authorizeCard(request: {
  ...
  const res = await fetch(`${API_BASE}/iso8583/authorize`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
